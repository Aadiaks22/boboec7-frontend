...
import { fetchReceiptNumber, fetchStudent, saveReceipt } from "@/http/api";
...
const FeeCollection = () => {
  ...
  // Query to fetch the receipt number
  const { data: receiptData, error: receiptError } = useQuery({
    queryKey: ['receiptNumber'],
    queryFn: fetchReceiptNumber,
    refetchOnWindowFocus: false,
  });

  //const { data: receiptData, error: receiptError } = useQuery<receiptNumber[]>(['receiptNumber'], fetchReceiptNumber);
  setReceiptNumber(receiptData.receiptNumber);

  const { id: r_id } = useParams<{ id: string }>(); // Extract the student ID from the route
  const [student, setStudent] = useState<Student | null | undefined>(null); // State to store student details
  ...
  const id: string | null = r_id || s_id;

  // Query to fetch the student details
  const { data, error: studentError } = useQuery({
    queryKey: ['student', id],
    queryFn: () => fetchStudent(id),  // Ensure this is a function that returns the result
    refetchOnWindowFocus: false,
  });

  //const { data, error: studentError } = useQuery(['student', id], () => fetchStudent(id));
  setStudent(data);
  ...
  // Mutation to save the receipt data
  const saveReceiptMutation = useMutation({
    mutationFn: (data: FormData) => saveReceipt(data), // Pass mutation function as an arrow function with `data` parameter
    onSuccess: () => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ['receiptNumber'] }); // Correct usage of invalidateQueries
      setIsSaved(true);
  ...
  if (receiptError) return <div>Error fetching receipt number</div>;
  if (studentError) return <div>Error fetching student data</div>;
